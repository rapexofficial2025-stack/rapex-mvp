  ...
  return <nav className="rapex-admin-sidebar" style={{ width: 252, height: "100vh", position: "sticky", top: 0, flexShrink: 0, display: "flex", flexDirection: "column", padding: "18px 12px", background: theme.colors.surface, borderRight: `1px solid ${theme.colors.border}` }}>
    ...
    <div style={{ flex: 1, display: "flex", flexDirection: "column", gap: 18, overflowY: "auto", paddingRight: 4 }}>
      {GROUPS.map((group, index) => {
        ...
        {group.label ? <button className="rapex-admin-group-title" type="button" aria-expanded={expanded} aria-controls={groupId} data-expanded={expanded} onClick={() => toggleGroup(group.label!)} style={{ color: theme.colors.textSecondary }}><span>{group.label}</span><span className="rapex-admin-group-chevron" aria-hidden="true">⌄</span></button> : null}
        <div id={groupId} className="rapex-admin-nav-collapse" data-expanded={expanded} aria-hidden={group.label ? !expanded : undefined}>
          ...
          return <button key={item.key} className={`rapex-admin-nav-item${active ? " is-active" : ""}`} type="button" tabIndex={expanded ? undefined : -1} aria-current={active ? "page" : undefined} onClick={() => onNavigate(item.path)} style={{ minHeight: 40, textAlign: "left", border: active ? `1px solid ${theme.colors.brandPrimary}` : "1px solid transparent", borderRadius: 8, padding: "10px 12px", backgroundColor: active ? `${theme.colors.brandPrimary}14` : "transparent", color: active ? theme.colors.textPrimary : theme.colors.textSecondary, font: "inherit", fontSize: 14, fontWeight: active ? 750 : 600, cursor: "pointer" }}>{item.label}</button>;
        })}</div>
        </div>
      </section>;
      })}
