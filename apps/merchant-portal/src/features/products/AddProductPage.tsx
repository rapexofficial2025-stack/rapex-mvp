...
export function AddProductPage() {
  ...
  return (
    <section className="merchant-product-page" style={styles.page}>
      <header className="merchant-product-page__header">
        <span className="merchant-product-page__eyebrow">Catalog · Product setup</span>
        <h1 style={{ ...styles.title, color: theme.colors.textPrimary }}>Add product</h1>
        <p style={{ color: theme.colors.textSecondary, margin: 0 }}>For {currentStore?.name ?? "your store"}. Changes are saved through the connected product service.</p>
      </header>

      <div className="merchant-product-page__card" style={{ ...styles.card, background: theme.colors.surface, borderColor: theme.colors.border }}>
        ...
        <LockableField label="Photo" locked={lockPhoto} onToggleLock={setLockPhoto} theme={theme}>
          <label style={styles.photoPicker}>
            {photoPreview ? (
              ...
              <span style={{ color: theme.colors.textSecondary, fontSize: 13 }}>Add product photo</span>
            )}
            <input
              type="file"
              accept="image/*"
      ...
      <h2 className="merchant-product-page__section-title" style={{ ...styles.sectionTitle, color: theme.colors.textPrimary }}>Your products</h2>
      {productsLoading ? (
        <Loading />
      ) : productsError ? (
        ...
        <div className="merchant-product-page__product-list" style={{ display: "flex", flexDirection: "column", gap: 10 }}>
